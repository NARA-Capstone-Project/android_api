...
class cict_db_users_functions
{
    ...
    public function checkAccStatus($id)
    {
        $stmt = $this->con->prepare("SELECT acc_status FROM accounts WHERE user_id = ?");
    ...
    {
        $stmt = $this->con->prepare("SELECT * FROM accounts WHERE user_id= ?;");
        $stmt->bind_param("s", $id);
